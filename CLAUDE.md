...
`ReceiverSignatureModal`), `components/users/` (`CreateUserModal`, `ChangeEmailModal`), and `components/settings/`
...
Sanctioned SECURITY DEFINER RPCs (the cross-cutting reads): `is_admin()`, `is_ops()`, `current_profile_id()`, `current_customer_id()`, `track_shipment()`, `dashboard_stats()`, `analytics_report()`, `admin_users_overview()`, `customer_balances_overview()`, `suggest_tracking_number()`, `public_settings()`, `admin_clear_pending_email_change()`. `dashboard_stats()` admits both roles but returns `null` for every money key unless the caller is an Admin, so the client shape stays stable; `customer_balances_overview(p_customer_ids)` follows the same shape (money keys null for non-Admins) and is deliberately scoped to one page of ids so the Customers list never scans every customer's shipments; `analytics_report()` is Admin-only outright, as is `admin_clear_pending_email_change(p_profile_id)` — it is the only sanctioned write to `auth.users`, and it touches the `email_change*` columns **only**. A secure email change that fails part way (GoTrue refusing either address) still records `auth.users.email_change` while leaving `email_change_sent_at` NULL and both token columns empty; that pending change is unredeemable but GoTrue keeps reporting it as `user.new_email`, so the console banner is permanent and every retry hits the same wall. Clearing it is what lets the change be requested again. Never write `auth.users.email` from SQL — the address is mirrored in `auth.identities.identity_data` and only the Admin API moves both, which is why `admin-set-email` exists rather than a second RPC. `supabase.auth.updateUser({ email })` stays the default path; the direct one is for the case it cannot serve, where the mail service will not deliver to the **current** address and so the confirm-from-both handshake can never complete. Retrying that flow is never the fix — on the built-in mailer the project gets ~2 sends/hour and a *failed* attempt spends them just as a successful one does, which is how a bad address turns into an afternoon of 429s.
