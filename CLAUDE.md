...
- Fix spacing/grammar/consistency automatically. UI is mobile-first and token-driven: deep navy (`navy-700`) primary, signal orange (`accent-500`) accent — the same palette as the live site. Never hardcode hex colors in components; change the theme in `tailwind.config.js` so both the public site and dashboard follow. The main conversion CTA uses `variant="accent"` on light backgrounds; avoid em-dashes in visible copy.
- Public-site visual standard (enterprise, not "AI-generated"): no gradient backgrounds or ambient blur "blob" glows; separate sections with hairline borders (`border-steel-100`) and whitespace. Hover states shift border/shadow only, never a `-translate-y` bounce. Lean on the token system (elevation, type scale) and photography, not decoration. Keep the navy dark bands and the low-opacity grid on `PageHero`.
...
**Auth & authorization** — `AuthContext` holds the Supabase session; being signed in is not enough. It reads the caller's own row from the `profiles` table (RLS: `auth_user_id = auth.uid()`) and exposes `profile`, `isAdmin`, `isOps`, `unauthorized`, `profileError`, and `refreshProfile()`. Roles are `Admin` | `Dispatcher` only. `isAdmin` is true only when `role === 'Admin' && status === 'Active'`; `isOps` is true for either role when Active, and mirrors the database's `is_ops()` RLS gate.
...
`ProtectedRoute` ([src/components/auth/ProtectedRoute.tsx](src/components/auth/ProtectedRoute.tsx)) **requires `isOps` for every route it wraps** (all of `/dashboard/*`), and `allow={['Admin']}` narrows the admin-only areas: Finance, Analytics, Users, Settings. Dispatchers get Overview, Shipments, Tracking, Customers, and Quotes. There is still no customer-facing portal — `customers` is a separate table with no dashboard access.
