  ...
  return (
    <Modal
      ...
      footer={
        <>
          ...
          <Button variant="deck" onClick={handleSave} loading={saving}>
            Save signature
          </Button>
        </>
      }
    >
      <div className="space-y-3">
        <p className="text-sm text-deck-500">Have the receiver sign below to confirm delivery.</p>
        <div className="relative overflow-hidden rounded-deck-sm border-2 border-dashed border-deck-150 bg-white">
          {loadingExisting ? (
            <div className="flex h-48 items-center justify-center">
              <Spinner className="h-5 w-5 text-deck-700" />
            </div>
          ) : existingUrl ? (
            <div className="flex h-48 flex-col items-center justify-center gap-2 bg-deck-50">
              <img src={existingUrl} alt="Captured receiver signature" className="max-h-32 object-contain" />
              <p className="text-xs text-deck-500">Already signed. Clear to re-sign.</p>
            </div>
          ) : (
            <canvas
              ref={canvasRef}
