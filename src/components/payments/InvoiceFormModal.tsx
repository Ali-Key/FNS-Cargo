...
import { round2, deriveVatRatePercent, invoiceSubtotal } from '@/utils/vat'
...
export function InvoiceFormModal({
  ...
  useEffect(() => {
    ...
    if (invoice) {
      ...
      const rate = deriveVatRatePercent(invoice)
      reset({
        shipment_id: invoice.shipment_id,
        // The form edits the pre-VAT charge; `amount` on the row is the net total.
        amount: invoiceSubtotal(invoice) as unknown as number,
  ...
  async function onSubmit(values: FormValues) {
    ...
    const payload = {
      ...
      // amount is the net payable total (charge + VAT - discount): it is what
      // payments are capped against by prevent_payment_overpay(), so VAT has to
      // be inside it for the counter to be able to collect it.
      amount: round2(parsed.amount + round2((parsed.amount * parsed.vat_rate) / 100) - parsed.discount),
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        {selected && (
          <div className="rounded-deck-sm border border-deck-150 bg-deck-50 px-4 py-3 text-sm">
            <p className="text-deck-600">
        ...
        <div className="rounded-deck-sm border border-deck-150 bg-deck-50 p-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 sm:items-start">
            ...
            <div className="flex flex-col gap-1.5">
              <span className="text-sm font-semibold text-deck-800">Grand total</span>
              <div
                aria-live="polite"
                className="flex h-11 items-center rounded-deck-sm border border-deck-150 bg-white px-3 font-mono text-lg font-bold tabular-nums text-deck-700"
              >
                {formatCurrency(grandTotal, 2)}
              </div>
            </div>
          </div>
          <p className="mt-2 text-xs text-deck-500">
            Grand total = charge + VAT − discount, and it is what the invoice bills and payments settle. Defaults to the rate in Settings but can be overridden
