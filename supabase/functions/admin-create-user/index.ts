// admin-create-user
// Creates a dashboard user (profiles.role = 'Admin'). Admin-only: the caller's
// JWT is verified against is_admin() before the service-role client provisions
// the auth account. The handle_new_auth_user trigger creates the profile from
// user_metadata, so we pass role + full_name there.
//
