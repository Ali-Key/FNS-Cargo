-- Reports: the reporting period only governed half the page.
--
-- revenue_trend, payment_stats, payment_mix, customer_growth and the transit
-- averages were all cut to the v_from window, but top_routes, method_mix,
-- cargo_mix, top_customers and every delivery counter read the whole table.
-- Switching 3M to 12M moved four sections and left six sitting on the same
-- all-time numbers, so the pills quietly meant "some of this page".
--
-- Worst of it was the tile row: "Collected" is period-scoped and "On-time
-- delivery" was not, so two figures measured over different spans of time sat
-- side by side reading as one row of facts.
--
-- This scopes the remaining sections to the same window. Nothing is added or
-- removed from the payload -- every key keeps its name and shape, so the client
-- needs no change.
--
-- The window is applied the way the rest of the page already applies it: by
-- when the thing happened. A shipment counts toward a lane, a mix or a customer
-- when it was booked in the period; a delivery counts as delivered, on time or
-- late when it landed in the period, which is the same placement the transit
-- average already uses. Only the two present-tense counters -- what is still
-- moving, and what is overdue -- are cut by booking date, because a shipment
-- still in transit has no completion date to be placed by.
--
-- Carried over unchanged from
-- 20260822213000_add_payment_stats_and_mix_to_analytics_report.sql otherwise.
